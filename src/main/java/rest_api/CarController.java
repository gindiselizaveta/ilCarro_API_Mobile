...
public class CarController implements Base_Api {
    ...
    public Response addNewCar(CarDto car) {
        return given().body(car).contentType(ContentType.JSON)
                .header("Authorization", tokenDto.getAccessToken()).when().post(BASE_URL + ADD_NEW_CAR_URL).thenReturn();
    }

    public Response addNewCar_WrongToken(CarDto car, String token) {
        return given().body(car).contentType(ContentType.JSON)
                .header("Authorization", token).when().post(BASE_URL + ADD_NEW_CAR_URL).thenReturn();
    }

    public Response addNewCar_WOToken(CarDto car) {
        return given().body(car).contentType(ContentType.JSON)
                .when().post(BASE_URL + ADD_NEW_CAR_URL).thenReturn();
    }

    public Response getAllUserCars() {
        return given().contentType(ContentType.JSON)
                .header("Authorization", tokenDto.getAccessToken())
                .when().get(BASE_URL + GET_ALL_USER_CARS_URL)
                .thenReturn();
    }
    ...
    public Response deleteCarBySerialNumber(String serialNumber) {
        return given()
                ...
                .header("Authorization", tokenDto.getAccessToken())
                .when().delete(BASE_URL + DELETE_CAR_URL + serialNumber)
                .thenReturn();
    }
}
